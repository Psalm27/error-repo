...
from .forms import LoginForm, StudentRegistrationForm, OrganizationRegistrationForm
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                if user.is_student:
                    return redirect('accounts:student_profile', student_id=user.id)
                elif user.is_organization:
                    return redirect('accounts:organisatin_profile', organization_id=user.id)
            else:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user = Student.objects.create_user(first_name=first_name, last_name=last_name,
                                               index_number=index_number, email=email,
